...
class nqueen {
    // Array to store left diagonal elements to check if queen can be placed in left
    // diagonal
    static int[] LeftDiagonal = new int[50];

    // Array to store right diagonal elements to check if queen can be places in
    // right diagonal
    ...
    // Function to return true and print if feasible solution is obtained else
    // return false
    ...
    static boolean NQueen(int chessBoard[][], int column, int N) {
        ...
        {
            // Check while placing a queen is not attacked by left and right diagonal
            // elements
            if ((LeftDiagonal[i - column + N - 1] != 1 &&
                    RightDiagonal[i + column] != 1) && cl[i] != 1) { // If the above condition is true then place the
                                                                     // queen
                chessBoard[i][column] = 1;
                LeftDiagonal[i - column + N - 1] = RightDiagonal[i + column] = cl[i] = 1;

                if (NQueen(chessBoard, column + 1, N))
                    return true;

                // If placing this queen in chessBoard doesn't lead to a correct & safe position
                // then remove queen from chessBoard
                // going back through BACKTRACKING
                chessBoard[i][column] = 0;
                LeftDiagonal[i - column + N - 1] = RightDiagonal[i + column] = cl[i] = 0;
            }
        }
